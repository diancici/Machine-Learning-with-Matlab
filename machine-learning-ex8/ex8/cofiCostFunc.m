...
% Vectorized implementation to compute J without regularization
...
% Implement the gradient with regularization
for i=1:num_movies
...
for j=1:num_users
    idx = find(R(:, j)==1); % list of all movies that have been rated by user j
    X_temp = X(idx,:);
    Y_temp = Y(idx,j);
